...
from om11.captchas import ReCaptchaV2, ReCaptchaV3, HCaptcha, GeetestV3, GeetestV4, Turnstile
...
class TestCaptchaDetection:
    @pytest.mark.parametrize("captcha_type,url", [
    ...
    def test_data_script_on_real_pages(self, page, captcha_type, url):
        ...
        assert isinstance(data, dict), f"Data script should return a dictionary for {captcha_type}"
    ...
    def test_submit_script_on_real_pages(self, page, captcha_type, url):
        ...
        if captcha_type in ["ReCaptchaV2", "ReCaptchaV3"]:
            value = page.evaluate('() => document.querySelector("#g-recaptcha-response").value')
            assert value == test_token
        elif captcha_type == "HCaptcha":
            value = page.evaluate('() => document.querySelector(\'[name="h-captcha-response"]\').value')
            assert value == test_token
        elif captcha_type == "Turnstile":
            value = page.evaluate('() => document.querySelector(\'[name="cf-turnstile-response"]\').value')
            assert value == test_token
...
class TestNegativeCases:
    def test_no_captcha_detection(self, page):
        ...
        for captcha_class in [ReCaptchaV2, ReCaptchaV3, HCaptcha, GeetestV3, GeetestV4, Turnstile]:
            captcha = captcha_class()
            result = page.evaluate(captcha.DETECT_SCRIPT)
            assert result is None or result is False, f"{captcha_class.__name__} incorrectly detected CAPTCHA"
